...
const AdminExperience: React.FC = () => {
  ...
  const handleSave = async () => {
    ...
    setSaving(true);
    try {
      const updated = await experienceService.update(editForm.id, editForm);
  ...
  return (
    <div className={`min-h-screen pt-24 pb-12 transition-colors duration-300 ${isDark ? 'bg-gray-900' : 'bg-gray-50'}`}> 
      <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        >
          <h1 className={`text-4xl sm:text-5xl font-bold mb-6 ${isDark ? 'text-white' : 'text-gray-900'}`}>Professional Experience</h1>
          <p className={`text-lg max-w-2xl mx-auto ${isDark ? 'text-gray-300' : 'text-gray-600'}`}>My journey through various roles in technology and engineering</p>
        </motion.div>
        {/* Experience Timeline */}
